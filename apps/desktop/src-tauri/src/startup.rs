...
const LOG_FILE_PREFIX: &str = "cursorok";
...
impl StartupDiagnostics {
    pub(crate) fn initialize() -> Result<Self, BoxError> {
        ...
        let filter = tracing_subscriber::EnvFilter::try_from_default_env()
            .unwrap_or_else(|_| "cursorok_desktop=info,cursor_server=info".into());
...
pub(crate) fn report_logging_failure(error: &(dyn Error + 'static)) {
    let details = error_chain(error);
    eprintln!("青云currsor小助手 failed to initialize logging: {details}");
...
fn show_fatal_dialog(details: &str, log_directory: Option<&std::path::Path>) {
    ...
    let description = format!(
        "青云currsor小助手无法启动 / failed to start.\n\n错误 / Error:\n{details}\n\n{log_guidance}"
    );

    let _ = MessageDialog::new()
        .set_level(MessageLevel::Error)
        .set_title("青云currsor小助手启动失败 / Startup Error")
